...
def runstreamlit():
    ...
    if uploaded_file is not None:
        ...
        with st.expander("Story"):
            st.write(story)
        st.audio("output.flac")


def main(argv):
   taskNum = ''
   streamlit = 'NO'
   opts, args = getopt.getopt(argv,"ht:s:",["task=","streamlit"])
   for opt, arg in opts:
      if opt == '-h':
         print ('test.py -t <tasknumber> -s <streamlitEnable>')
         sys.exit()
      elif opt in ("-t", "--task"):
         taskNum = arg
         match taskNum:
            ...
            case _:
                ...
                subprocess.run(["ls", "-l"]) 
      elif opt in ("-s", "--streamlit"):
         streamlit = arg
         print ('Runnung streamlit....')
         subprocess.run(["ls", "-l"]) 
         runstreamlit()
